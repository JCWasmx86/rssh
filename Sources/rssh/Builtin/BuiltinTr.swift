...
struct BuiltinTr: ParsableCommand, BuiltinCommand {
  ...
  @Flag(name: [.customShort("d"), .customLong("delete")], help: "Delete character") var d: Bool =
    false
  @Option(name: .shortAndLong, help: "Use complement of SET1.") var complement: Bool = false
  @Argument(help: "The character set to translate from.") var set1: String
  ...
  public func apply(stdin: String) throws -> String {
    ...
    if let s = self.set2 { set2 = s.unicodeScalars } else { set2 = set1 }
    var res = ""
    for scalar in stdin.unicodeScalars {
      if self.d {
        if !set1.contains(scalar) { res.unicodeScalars.append(scalar) }
      } else {
        let index = set1.firstIndex(of: scalar)
        if self.complement {
          if index != nil { res.unicodeScalars.append(set2[index!]) }
        } else {
