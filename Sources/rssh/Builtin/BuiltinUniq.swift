...
struct BuiltinUniq: ParsableCommand, BuiltinCommand {
  ...
  @Flag(name: .shortAndLong, help: "Suppress repeated lines.") var unique: Bool = false
  ...
  @Flag(name: .shortAndLong, help: "Ignore differences in case when comparing") var ignoreCase:
    Bool = false
  ...
  func apply(stdin: String) throws -> String {
    ...
    task.arguments = []
    if self.unique { task.arguments!.append("-u") }
    if self.repeated { task.arguments!.append("-d") }
    if self.count { task.arguments!.append("-c") }
    let stdinPipe = Pipe()
